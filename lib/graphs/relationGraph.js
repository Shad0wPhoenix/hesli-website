...
function resolveOrCreateStub(contentGraph, target) {
    ...
    console.info(`${kleur.gray(`[RelationGraph]`)} Stub created for ${kleur.blue(`${toLabel(target)}`)}`);
 ...
 * ─── Relation-type guard ─────────────────────────────────────────────────────
 *
 * Before doing any type-based resolution, this function checks whether the
 * relation key itself already has an explicit definition in relationTypes.js
 * (i.e. it defines its own `inverse`, or is a shorthand string). If it does,
 * that definition fully describes the relation on its own — via
 * normaliseRelation — and this function returns null immediately.
 *
 * Without this guard, an article whose own `type` happens to have a
 * definition (e.g. `type: character`) would have that definition's `inverse`
 * silently override an unrelated, explicitly-defined relation like
 * `currentResidence`, purely because both the relation type and the article
 * type are resolved independently and the caller checks `type?.inverse`
 * before `relation.inverse`. The guard ensures relation-specific definitions
 * always take priority over the source article's general type definition.
 *
 * Example of the bug this prevents:
 *   An article with `type: character` and a `currentResidence: Limani`
 *   relation should produce a "Current Residence" / "Currently Resides"
 *   backlink — not the generic "Related To" backlink defined on `character`,
 *   even though `character` has its own type definition.
 *
 ...
 *   1. The relation key has its own explicit definition (guard above) —
 *      caller falls back to relation.inverse / relation.inverseLabel.
 *   2. No source type can be determined at all — caller falls back to the
 *      base relation definition the same way.
 *
...
function resolveTypeDefinition(target, relation, sourceNode, targetNode, relationType, definitions) {
    const relationDef = definitions?.[relationType];
    const hasOwnDef = relationDef && (typeof relationDef === "string" || relationDef.inverse);
    const shouldDefer = typeof relationDef === "object" && relationDef?.deferToSourceType;

    if (hasOwnDef && !shouldDefer) return null;

    const sourceType = normaliseTypeKey(
        target.type ?? relation.type ?? sourceNode.page?.data?.type ?? null
    );

    if (shouldDefer && !sourceType) {
        console.warn(
            `${kleur.gray(`[RelationGraph]`)} ${kleur.blue(`"${sourceNode.name}"`)} uses ${kleur.yellow(`"${relationType}" (deferToSourceType)`)} but has ${kleur.red(`no \`type\``)} set — falling back to the generic ${kleur.yellow(`"${relationType}"`)} definition.`
        );
    }
    ...
    if (shouldDefer && !definition) {
        console.warn(
            `${kleur.gray(`[RelationGraph]`)} ${kleur.blue(`"${sourceNode.name}"`)} has type ${kleur.yellow(`"${sourceType}"`)} with ${kleur.red(`no entry`)} in \`relationTypes.js\` — its ${kleur.yellow(`"${relationType}"`)} backlinks will use the generic definition instead.`
        );
    }
    ...
    if (shouldDefer && !edgeOverride && sourceType !== relationType) {
        console.info(
            `${kleur.gray(`[RelationGraph]`)} ${kleur.blue(`"${sourceNode.name}"`)} (type: ${kleur.green(`${sourceType}`)}) using relation ${kleur.yellow(`"${relationType}"`)} has ${kleur.red(`no byTargetType.${targetType} override`)} — falling back to ${kleur.green(`${sourceType}`)}'s base definition ${kleur.yellow(`(inverse: "${definition.inverse}")`)}. Verify this is the intended label.`
        );
    }
