...
function normaliseRelation(relationType, value, definitions, sourceNode) {
    ...
    console.log(`[RelationGraph]Creating relation for ${sourceNode.name} with relationType "${relationType}"\r\n
            Value: ${value}\r\n
            FallbackLabel: ${fallbackLabel}\r\n
            EdgeOverride: ${edgeOverride}\r\n
            Definition Label: ${definition?.label}\r\n
            Default Label: ${toLabel(relationType)}\r\n`);
    ...
    for (const node of contentGraph.flat) {
        ...
        for (const [relationType, relationValues] of Object.entries(rawRelations)) {
            ...
            for (const target of relation.targets) {
                ...
                ensureBucket(targetNode.backlinks, inverseKey, inverseLabel, inverseLabelPlural);
