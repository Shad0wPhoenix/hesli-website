...
export default function buildContentGraph(pages) {
    ...
    for (const item of pages) {
        ...
        if (nodesByTitle.has(parent.name)) {
            ...
            console.warn(`${kleur.gray(`[ContentGraph]`)} Duplicate title ${kleur.blue(`"${parent.name}"`)} - ${kleur.cyan(`"${existing.url}"`)} and ${kleur.yellow(`"${parent.url}"`)} both use this name. Title-based relation lookups will resolve to ${kleur.yellow(`"${parent.url}"`)}`);
